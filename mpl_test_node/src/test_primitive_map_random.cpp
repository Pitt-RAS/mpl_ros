...
int main(int argc, char** argv) {
  ...
  std::ifstream inObst("/home/andrew/obstacles.txt");
  ...
  while(std::getline(inObst, obst) and ros::ok()) {
    std::vector<std::string> obstFields;
    boost::split(obstFields, obst, boost::is_any_of( "," ));
    ...
    for (int i=0; i<4; i++) {
      ...
      new_obstacle.odom.pose.pose.position.x = std::stof(obstFields[i]);
      new_obstacle.odom.pose.pose.position.y = std::stof(obstFields[i+1]);
    ...
    for (auto& obstacle : obstacles.obstacles) {
      ...
      for (pz = voxel_map_origin[2]; pz <= pipe_height; pz += 0.1) {
        for (float theta = 0; theta < 2 * M_PI; theta += 0.15) {
          for (float r = pipe_radius; r < pipe_radius; r += map.resolution) {
    ...
    std::ifstream inFile("/home/andrew/input.txt");

    while(std::getline(inFile, line) and ros::ok())
    {
      // Set start and goal
      ...
      start_x = std::stof(fields[0]);
      start_y = std::stof(fields[1]);
      start_z = std::stof(fields[2]);
